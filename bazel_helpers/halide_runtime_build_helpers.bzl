...
def _gen_runtime_cpp_component_3(component_file, bits, suffix = "") :
  native.genrule(
    ...
    tools = [ "//tools:bitcode2cpp" ],
    srcs = [ "initmod.{0}_{1}{2}.bc".format(component_file, bits, suffix), ],
    cmd = "$(location //tools:bitcode2cpp) {0}_{1}{2} < $< > $@".format(component_file, bits, suffix),
...
def _gen_runtime_ll_component_2(component_file) :
  native.genrule(
    ...
    cmd = "$(location //tools:bitcode2cpp) {0}_ll < $< > $@".format(component_file),
...
def _gen_runtime_nvidia_bitcode_component(component_file) :
  native.genrule(
    ...
    cmd = "$(location //tools:bitcode2cpp) ptx_{0}_ll < $< > $@".format(component_file),
